...
# === 网页配置 ===
st.set_page_config(page_title="亚马逊广告全能王 (去重修复版)", layout="wide", page_icon="🛡️")
st.title("🛡️ Amazon 广告优化全能王 (完美运行版)")
st.success("💡 已修复：自动去除表格中的重复列名！")
...
if file_bulk:
    try:
        ...
        if bulk_df.empty:
            st.error("❌ 没找到包含'关键词文本'的数据表！请检查文件。")
        else:
            st.success(f"✅ 成功锁定数据表: 【{found_sheet_name}】")
            ...
            st.subheader("1️⃣ 竞价优化建议")
            if not bad_bids.empty:
                st.dataframe(bad_bids[['Campaign', 'Target', 'Spend', 'ACoS', 'Max Bid', '建议新竞价']].head(10))
            else:
                st.success("✅ 竞价表现良好。")

    except Exception as e:
        st.error(f"Bulk 文件处理出错: {e}")
...
if file_term:
    try:
        ...
        st_col_map = {
            ...
            '广告活动名称': 'Campaign', '广告活动名称': 'Campaign',
        ...
        if 'Orders' in term_df.columns and 'Clicks' in term_df.columns:
            ...
            st.subheader("2️⃣ 否词建议 (无效流量)")
            if not neg_candidates.empty:
                st.error(f"🚨 发现 {len(neg_candidates)} 个只烧钱不出单的词！")
                st.dataframe(neg_candidates[['Campaign', 'Search Term', 'Clicks', 'Spend']].head(10))
                
                output = io.BytesIO()
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    neg_candidates.to_excel(writer, index=False)
                st.download_button("📥 下载否词表", output, "Negative_Keywords.xlsx")
            else:
                st.success("✅ 搜索词很干净。")
        else:
            st.error(f"❌ 列名不匹配！检测到的列名：{list(term_df.columns)}")

    except Exception as e:
        st.error(f"搜索词报告出错: {e}")
    ...
    if st.button("开始 AI 分析"):
        if not deepseek_key:
            ...
            if 'bad_bids' in locals() and not bad_bids.empty:
                ai_bid_data = bad_bids[['Target', 'ACoS', 'Spend']].head(5)
